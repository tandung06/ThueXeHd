...
@section('title', 'Đăng ký - Thuê Xe HD')
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-xs w-full space-y-6">
        ...
        <div class="text-center">
            ...
            <h2 class="text-xl font-bold text-gray-800">Tạo tài khoản</h2>
        ...
        <div class="bg-white rounded-xl shadow-lg p-5">
            <form class="space-y-4" method="POST" action="{{ route('auth.register') }}">
                ...
                <!-- Driver License -->
                <div>
                    <input id="driver_license_no" 
                           name="driver_license_no" 
                           type="text" 
                           value="{{ old('driver_license_no') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('driver_license_no') border-red-500 @enderror"
                           placeholder="Số bằng lái xe (tùy chọn)">
                ...
                <div>
                    <textarea id="address" 
                              ...
                              placeholder="Địa chỉ (tùy chọn)">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
